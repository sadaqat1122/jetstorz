...
def logout(request):
    ...
    # Add your logout logic here (e.g., clearing sessions, etc.)
...
def activate(request, uidb64, token):
    ...
    except(TyperError, ValueError, OverflowError, Account.DoesNotExist):
        user = None

    if user is not None and default_token_generator.check_token(user, token):
        user.is_active = True
        user.save()
        messages.success(request, 'congrulationsD! your account is active')
        return redirect('login')
    else:
